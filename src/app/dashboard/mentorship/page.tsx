...
import type { MentorshipRequestWithStudent } from "@/types";
...
type StudentRequest = MentorshipRequestWithStudent;
...
function Avatar({
  ...
  return (
    <div
      style={style}
      className="relative flex-shrink-0 overflow-hidden rounded-2xl border border-slate-700 bg-slate-800"
    >
      {image ? (
...
export default function MentorshipPage() {
    ...
    supabase
      ...
      .select(
        `id, student_id, alumni_id, status, created_at,
         student:profiles!mentorship_requests_student_id_fkey(id, full_name, profile_image, department, faculty, graduation_year)`
      )
      .eq(column, user.id)
      ...
      .then(({ data }) => {
        if (data) {
          setRequests(
            (data as unknown as RawRequest[]).map((r) => ({
              id: r.id,
              ...
              student: {
                ...
                fullName: r.student?.full_name ?? "Unknown",
                profileImage: r.student?.profile_image ?? undefined,
                department: r.student?.department ?? "",
                faculty: r.student?.faculty ?? "",
                graduationYear: r.student?.graduation_year ?? 0,
              },
            }))
          );
        }
  ...
  const handleAccept = async (req: StudentRequest) => {
    if (!user) return;
    setActionId(req.id);
    const supabase = createClient();
                ...
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div className="flex items-center gap-4">
                    <Avatar
                      ...
                      image={req.student.profileImage}
                    />
                    <div>
                      <Link
                        href={`/alumni/${req.student.id}`}
                      ...
                      <div className="mt-0.5 flex flex-wrap gap-2 text-xs text-slate-500">
                        ...
                        {req.student.faculty && (
                          <span>· {req.student.faculty}</span>
                        )}
                        {req.student.graduationYear > 0 && (
                          <span>· Class of {req.student.graduationYear}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    <span
                      className={`rounded-full px-3 py-1 text-xs font-medium capitalize ${statusBadge[req.status]}`}
                    >
                      {req.status}
                    </span>
